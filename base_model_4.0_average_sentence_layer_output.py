...
tf.flags.DEFINE_string("log_dir", "./base_model_4.0_average_rnn_output/log/", "log_dir")
tf.flags.DEFINE_string("log_dir_emb", "./base_model_4.0_average_rnn_output/log/word_embedding/",
                       "visualize_embeddings_log_dir")
tf.flags.DEFINE_string("checkpoint_dir", "./base_model_4.0_average_rnn_output/checkpoint/", "checkpoint_dir")
tf.flags.DEFINE_string("train_data_dir", "./data/", "train_data_dir")
tf.flags.DEFINE_string("output_dir", "./base_model_4.0_average_rnn_output/output/", "output_dir")
...
# tf.flags.DEFINE_float("coefficient_cos_2", 0.5, "coefficient for cos_2 in loss function")
...
tf.flags.DEFINE_integer("epoch_size", 10, "epoch_size. Default 10")
...
class BaseModel(object):
    def __init__(self, question_vocab_size, answer_vocab_size,
                 ...
                 name="base_model"):
        """
        Creates an End-To-End Q-A matching base_model
        ...
        self.label = tf.placeholder(tf.float32, [self._batch_size, NUM_ANSWER_FOR_PER_QUESTION],
                                    "label_with_coefficient")  # --loss_function with coefficient
        ...
        with tf.variable_scope("base_model"):
            ...
            with tf.variable_scope("question_sentence_gru"):
                ...
                question_sentence_gru_outputs, question_sentence_gru_state = tf.nn.dynamic_rnn(
                    ...
                    dtype=tf.float32)
            with tf.variable_scope("answer_sentence_gru"):
                answer_sentence_gru_outputs, answer_sentence_gru_state = tf.nn.dynamic_rnn(
                    self._answer_sentence_cell, answer_word_emb, self.answer_length,
                    dtype=tf.float32)
            ...
            context_q = tf.reduce_sum(question_sentence_gru_outputs * tf.expand_dims(question_mask, axis=2),
                                      axis=1) / tf.reduce_sum(question_mask, axis=1, keep_dims=True)
            print("--context_q-- ", context_q)
            context_a = tf.reduce_sum(answer_sentence_gru_outputs * tf.expand_dims(answer_mask, axis=2),
...
def main():
    with tf.variable_scope("base_model"):
        model = BaseModel(VOCAB_SIZE_QUESTION, VOCAB_SIZE_ANSWER, FLAGS.batch_size, FLAGS.word_embedding_size,
    ...
    with tf.Session(config=config) as sess:
        ...
        saver.restore(sess, FLAGS.checkpoint_dir + "base_model_ckpt-118980")  # ----------
        ...
        print("version_4.0_base_model_4.0 opt=Momentum(0.03, 0.9). decrease_rate=0.3 with projector")
        print("UPDATE_1: learning_rate decrease only when (precise_test - precises_test[-2]) < 1e-3")
        print("UPDATE_2: sentence_representation = average_sentence_layer_output")
        print("dropout=0.5, max_grad_norm=5.0, no word_GRU_layer.")
        print("loss_function: max(0, 1.0-(cos_1-cos_2)), sentence_representation=average_sentence_layer_output*****")
        print("Train_set checkpoint. save_path: ", save_path)  # ----------

        base_model_learning_rate = FLAGS.learning_rate
        precises_train = [0]  # ----------
        precises_test = [0]  # ----------
        for epoch in range(0, FLAGS.epoch_size):  # ----------
            print('**************epoch************************', epoch)
            print("learning_rate is ", base_model_learning_rate)
            ...
            for batch in range(training_dataset_loader.num_batch):  # num_batch 11898
                ...
                loss_mean_train, precise_num_batch_train, _, summary_merge, similarity_per_batch_train, logits_train = \
                    sess.run([batch_loss, precise_indices, train_op, summary_merge_op, similarity_per_batch_, logits_],
                             feed_dict={model.question: question_batch,
                                        ...
                                        model.label: np.asarray([[1.0, -1.0] for _ in range(FLAGS.batch_size)],
                                                                np.float32),
                                        ...
                                        model.learning_rate: base_model_learning_rate})
                ...
                if (global_step % training_dataset_loader.num_batch) == 0:  #
                    ...
                    if not os.path.exists(save_path):
                        ...
                        print("dir exists")
                    saver.save(sess, save_path + "base_model_ckpt", global_step=global_step)
                ...
                if global_step % training_dataset_loader.num_batch == 0:  # epoch % FLAGS.evaluation_interval == 0:
                            ...
                            sess.run(
                                ...
                                feed_dict={model.question: question_batch_test,
                                           ...
                                           model.label: np.asarray(
                                               [[1.0, -1.0] for _ in range(FLAGS.batch_size)],
                                               np.float32),
                    ...
                    with codecs.open(FLAGS.output_dir + "precise_temp.txt", "a", "utf-8") as temp_file:
                        temp_file.write(" , precise_test=" + str(precise_test) + " , learning_rate=" +
                                        str(base_model_learning_rate) + "\n")

                    if (precise_test - precises_test[-2]) < 0:
                        print('this epoch gets worse result.')
                    if ((precise_test - precises_test[-2] < 1e-3) and
                            (0.001 < base_model_learning_rate)):
                        base_model_learning_rate = base_model_learning_rate * FLAGS.decrease_rate
                        print('epoch ', epoch, ' done. update model.learning_rate to ', base_model_learning_rate)
                    ...
                    print("epoch {} base_model done. Saving csv.".format(epoch))
                    ...
                    df_precises_batch.to_csv(FLAGS.output_dir + "precises_batch_epoch_{}_base_model.csv".format(epoch))
                    print("csv save done.")
        ...
        df_precises.to_csv(FLAGS.output_dir + "precises_base_model" + ".csv")
        print('ALL DONE.')
